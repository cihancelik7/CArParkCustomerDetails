...
class SQLHelperForGLJournals(context: Context) :
    ...
    fun insertJournal(glJournal: GLJournalsModel): Long {
        ...
        if (glPeriod != null) {
            val periodName = glPeriodsHelper.getGLPeriodNameById(glJournal.periodId)
            if (periodName != null) {
                val updatedStatus = "Updated Status: $periodName"
        ...
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS GL_JOURNALS(PERIOD_ID INTEGER,JOURNAL_DATE DATE, STATUS TEXT," +
                    "AMOUNT TEXT, UPDATE_DATE DATE, CREATION_DATE DATE)"
        )
    ...
    fun getAllJournal(): ArrayList<GLJournalsModel> {
        ...
        var status: String
        var amount: String // Değişiklik yapıldı
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                amount = cursor.getString(cursor.getColumnIndex("AMOUNT")) // Değişiklik yapıldı
    ...
    fun deleteGLJournalById(id: Int): Int {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        contentValues.put("JOURNAL_ID", id)

        val success = db.delete("GL_JOURNALS", "JOURNAL_ID=$id", null)
        db.close()
        return success
    }

    fun getGLJournalById(journalId: Int): GLJournalsModel? {
        val db = this.writableDatabase
    ...
    fun getGLJournalNameById(journalId: Int): String? {
        val db = this.writableDatabase
        val selectQuery = "SELECT STATUS FROM GL_JOURNALS WHERE JOURNAL_ID = $journalId"
